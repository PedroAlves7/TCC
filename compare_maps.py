...
def test_match(map_name, image_name, tolerance, black_threshold):
    ...
    white_pixels = np.sum(im_free >= 254)
    white_pixels_model = np.sum(im_model_raw >= 254)

    if white_pixels_model > 0:
        ...
        print('Model has no white pixels to compare free space.')
...
if __name__ == "__main__":
    # Linha do ROS removida: rospy.init_node('icp_map_comparison_node')
    ...
    # Linha alterada: de 'args, _ = parser.parse_known_args()' para:
